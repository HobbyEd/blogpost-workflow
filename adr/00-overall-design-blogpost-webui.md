# 00. Overall Design Document: Blogpost Workflow Web UI & Agent Platform
...
### 3.1 Refactoring van `scripts/orchestrate.py` naar `orchestrator/` Package (Gerealiseerd ✅)
Om de CLI en de Web UI exact dezelfde bedrijfslogica te laten delen, is de voormalige 1.600+ regels grote `orchestrate.py` opgesplitst in een modulair Python package:
...
└── orchestrator/                 # Python package
    ...
    └── service.py                # WorkflowService (Pure Python API voor CLI & Backend)
```

---

## 4. Toekomstperspectief & Extensies

### 4.1 RAG Archief Vectorstore (Fase D)
In een volgende ontwikkelfase wordt de `blogpost-onderzoeker` voorzien van een lokale RAG-index over het gehele archief van gepubliceerde artikelen op `edwinvandillen.nl`. Hierdoor kan de onderzoeker automatisch verwijzen naar relevante eerdere artikelen en beproefde concepten.

### 4.2 Integratie met Jarvisje.com Digital Twin
De Web UI vormt de opstap naar integratie met het bredere **Jarvisje.com** platform: de personal AI assistant en digitale tweeling van Edwin van Dillen.

---

## 5. Implementatie-Fasering

| Fase | Onderdeel | Status | Omschrijving |
|---|---|---|---|
| **Fase A** | Service Refactoring | ✅ **Gerealiseerd** | Opsplitsen `orchestrate.py` naar `scripts/orchestrator/` package met pure `WorkflowService` Python API en 46 unit-tests. |
| **Fase B** | FastAPI Web Backend | 🎯 **Aanstaande** | REST API endpoints & WebSocket SSE event-streamer opzetten in Python. |
| **Fase C** | Web UI Frontend | ⏳ Gepland | React / Single-Page Dashboard met Socratische Chat & Visuele Stepper. |
| **Fase D** | RAG Vectorstore | 🔮 Toekomst | Geautomatiseerde archief-indexering voor de `blogpost-onderzoeker`. |
